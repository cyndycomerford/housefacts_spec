...
|:-----|:------|:---------|:------------|
| building_id | string | Yes | Building_id is a unique alpha-numeric identifier created or assigned by a single authoritative agency in a locale (e.g. tax collector)  that identifies each residential building inspected by any agency in locality. See source and definition of building_id in feed.csv |
...
| predir | string | No | Field for the Prefix of N(orth), E(ast), S(outh) or W(est) that precedes the street name |
